...
func _process(delta: float) -> void:
  ...
  if current_turn_state == TurnStates.ROLL_DICE:
    ...
    _state_battle_check()
  elif current_turn_state == TurnStates.BATTLE_RESULTS:
    _state_battle_results()
  elif current_turn_state == TurnStates.MINIGAME_CHECK:
    _state_minigame_check()
  elif current_turn_state == TurnStates.MINIGAME_RESULTS:
    _state_minigame_results()
...
func _state_battle_check() -> void:
  ...
  if len(battle_between) == 1:
    ...
    last_bus_idx = main_bus.len()
    current_turn_state = TurnStates.BATTLE_RESULTS

func _state_battle_results() -> void:
  current_turn_state = TurnStates.MINIGAME_CHECK
  var messages = main_bus.poll(last_bus_idx)
  last_bus_idx = main_bus.len()
  prints("Battle minigame result:", messages[0]['data'])
...
func _state_minigame_check() -> void:
  if current_player_idx == len(player_nodes) - 1:
    ...
    last_bus_idx = main_bus.len()
    current_turn_state = TurnStates.MINIGAME_RESULTS
  else:
    current_turn_state = TurnStates.END_OF_TURN

func _state_minigame_results() -> void:
  current_turn_state = TurnStates.END_OF_TURN
  var messages = main_bus.poll(last_bus_idx)
  last_bus_idx = main_bus.len()
  prints("Minigame result:", messages[0]['data'])
